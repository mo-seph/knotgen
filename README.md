...
Controls: **drag** rotates, **scroll / two-finger drag** zooms, and a button row along the bottom covers everything (each button also has a key): `t` the axis triad (Fusion colours, X red / Y green / Z blue; off by default), `m` the green **start markers** (where the path begins — also where the sweep seam, connector 1 and Segment 1 sit, with an arrow showing the direction the numbering runs; off by default), `p` the opaque shaded **tube** (drawn whenever `--tube` is given; hidden by default when a strip is shown), `n` the strip's LED-normal tick lines, `g` the axes/grid, `1`/`2`/`3` the strip colourings, and `q` / **Cmd-Q** closes the window. Red vertical lines mark crossings on flat (2.5D) designs.
