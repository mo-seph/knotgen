...
- **max tube diameter** — the binding constraint of the two, with safety factors, and *which* one binds (`limited by bend radius` / `limited by strand gap`) so you know which knob helps. The GUI also marks the tight spots on the model: orange rings = tight turns, red spheres = tight gaps (toggleable).

If `--tube` fails the check, nothing is exported and the message says which knob to turn (usually `--depth`, `--width`, or `--tightness`).

When no combination of knobs works, add **`--relax`**: an opt-in optimisation pass (strand repulsion + bend relief, in the spirit of ideal-knot tightening in reverse) that redistributes space at fixed overall size until the requested tube fits. It's topology-safe (every step is capped well below the current strand gap, so strands can't pass through each other — the crossing diagram is verified unchanged in tests), preserves exact n-fold symmetry via spectral projection, prints progress every few iterations, and **returns the best state seen — never worse than your original layout**. It stops early when progress plateaus and reports the tube size the layout tops out at. Effort is steered automatically toward whichever constraint is binding — gap-limited knots get more repulsion, curvature-limited ones get more un-kinking, and the progress lines say which regime you're in. The step size is adaptively damped (constant gains oscillate), a gentle curve-shortening flow runs throughout so relaxation can't pump surplus length into the curve as wrinkles, and when you gave an explicit `--depth`, relaxation **holds that depth by default** — an explicit depth is a design decision, not a suggestion (the budget is a soft force, so depth-limited relaxation makes real progress instead of fighting a per-iteration squash). Companions: `--relax-max-depth MM` sets a different z budget (`0` = unlimited, the old free-inflation behaviour); `--relax-max` grinds much harder before giving up; `--relax-iterations N` caps the budget (default 150, rarely reached).

Under the hood, relaxation is **fixed-rope** by default (`--relax-method sono`, after the SONO ideal-knot algorithm): the rope is treated as inextensible, overlaps — measured by the **Gonzalez–Maddocks tangent-point radius**, the quantity whose infimum is exactly the fattest tube a curve can carry — are resolved by *moving* rope, the curve is projected back onto its length budget every step (curve-shortening, which spends wiggles before lobes), and the working tube is inflated 3% each time the design is clean. Earlier relaxers bought clearance by *manufacturing* rope, and surplus rope confined to a slab has one place to go: wiggles and coils — the fixed-rope method makes those impossible by construction. `--rope-slack` (default 0.10) is the only extra rope allowed, for rounding crushed kinks. `--relax-method gm` keeps the earlier ambition-driven dynamics and `forces` the original two-force pair.

Watch it work with `--relax-snapshots DIR` (a PNG every `--snapshot-every` iterations plus a `trace.json` of gap / bend / fits / rope length) or `--relax-gif FILE`; in the GUI a relax run streams its snapshots into the viewer as it goes, and afterwards a **timeline slider** scrubs the whole run — pick any intermediate state as *the* design with "keep this state" (tightening trades elegance for tube size; this lets you choose the point on that curve yourself), or download the run as a looping GIF. Every relax run finishes with an **adaptive spectral polish**: high-frequency wobble is damped pass by pass while the achievable tube holds (and often grows — kinks are high-frequency content too); `--polish` applies the same pass without relaxing. The [`gm` module](src/knotgen/gm.py) also exposes `thickness()` directly. Two useful properties of the optimisation: the result **does not depend on the tube size you asked for** (internally it always chases ~2× whatever currently fits, so a 14 mm and a 25 mm request relax the same design identically — the request only decides pass/fail), and it **keeps opening and smoothing past a satisfied request** until it genuinely plateaus, so lobes fill the available depth instead of stopping pinched at "barely fits". A relax run is never wasted: if the result doesn't land in a `--out`, the curve is parked in `output/relaxed_<name>.json` — rerun checks, previews or meshes from it without re-relaxing (`knotgen check output/relaxed_5_1.json --tube 16 --mesh lamp.stl`). When a check fails but you want the file anyway (it's close, or you'll fix it in CAD), `--force` writes it and records the failing check in the JSON. Without `--relax`, layouts are exactly as designed. (It can't rescue a deliberately crushed `--depth` on a fully-3D ideal conformation — give those their natural depth.) Note that a non-circular profile needs clearance for its *diagonal*, not its width — re-check with `knotgen check file.json --tube <diagonal>`.
...
Controls: **drag** rotates, **scroll / two-finger drag** zooms, and a button row along the bottom covers everything (each button also has a key): `t` the origin marker (axis triad in Fusion colours, X red / Y green / Z blue; off by default), `m` the green **start markers** (where the path begins — also where the sweep seam, connector 1 and Segment 1 sit, with an arrow showing the direction the numbering runs; off by default), `p` the opaque shaded **tube** (drawn whenever `--tube` is given; hidden by default when a strip is shown), `n` the strip's LED-normal tick lines, `g` the grid, `1`/`2`/`3` the strip colourings, and `q` / **Cmd-Q** closes the window. Red vertical lines mark crossings on flat (2.5D) designs.
...
**Browse the catalogue** opens a graphical index: small drawn previews of every knot and link (over/under crossings included, one colour per component), grouped by family — classic 3₁–8₂₁, torus, weaving, the 9–11 crossing tables, and the links by crossing count. Click any preview to generate it. Each group links to a short **"what the names mean"** page explaining its numbering system with history and examples — those pages are markdown in [`src/knotgen/docs/`](src/knotgen/docs/) ([classic](src/knotgen/docs/classic.md) · [9–11 crossings](src/knotgen/docs/ht.md) · [links](src/knotgen/docs/links.md) · [torus](src/knotgen/docs/torus.md) · [weaving](src/knotgen/docs/weaving.md)), readable on GitHub too. A sixth page, [How knotgen represents a knot: Fourier series](src/knotgen/docs/fourier.md), explains the representation everything here is built on.
...
- **Scale factor** — dialog value (default 1) applied numerically to all imported geometry (path, strip, connector/mount positions) but not to your profile, connector components, or label text — those stay physical size. Preflight numbers in the JSON refer to scale 1, so rescale them mentally (clearances scale linearly).
- **User parameters for later tweaking** — the import also creates two unitless user parameters (Modify → Change Parameters): **`knotScale`** drives a Scale feature over the knot's geometry sketches (path + strip), and **`knotProfileScale`** drives one on the copied profile sketch (initialised from the "Profile scale" dialog value) — edit either afterwards and the lofts/sweeps rebuild. Caveat: connector, label and mount *placements* are static transforms and won't follow a `knotScale` change — re-run the import stages for those after a big change.
- **`central mount point`** — every import adds a sketch of that name with a circle at the component origin, as a convenient target for joints/grounding.
